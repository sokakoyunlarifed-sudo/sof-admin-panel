...
export default function ProjectsListClient({ initial, role }: { initial: ProjectRow[]; role: Role }) {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {(role === "admin" || role === "superadmin") && (
          <Link href="/content/projects/new" className={cn(buttonVariants({ variant: "primary", shape: "rounded", size: "small" }))}>
            New Project
          </Link>
        )}
      </div>
              ...
              <tr key={n.id}>
                <td className="px-4 py-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-dark-6">{n.slug}</div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-2">
                ...
                <td className="px-4 py-2 text-right">
                  <div className="flex justify-end gap-2">
                    <Link href={`/content/projects/${n.id}/edit`} className={cn(buttonVariants({ variant: "outlineDark", shape: "rounded", size: "small" }))}>
                      Edit
                    </Link>
                    {(role === "admin" || role === "superadmin") && (
                      n.published_at ? (
